...
JugadorView::JugadorView(){
  ...
  auto boundsP = animationP->getSprite().getLocalBounds();
  auto boundsT = animationT->getSprite().getLocalBounds();
  animationP->getSprite().setOrigin({boundsP.size.x / 2.0f, boundsP.size.y / 2.0f});
  animationT->getSprite().setOrigin({boundsT.size.x / 3.0f, boundsT.size.y / 2.0f});
...
void JugadorView::manejarEventos() {
  //mover en eje y
  if (!down && sf::Keyboard::isKeyPressed(sf::Keyboard::Key::W))
    up = true;
  else if (!up && sf::Keyboard::isKeyPressed(sf::Keyboard::Key::S))
    down = true;
  else {up = false; down = false;}
  //mover en eje x
  if (!right && sf::Keyboard::isKeyPressed(sf::Keyboard::Key::A))
    left = true;
  else if (!left && sf::Keyboard::isKeyPressed(sf::Keyboard::Key::D))
    right = true;
